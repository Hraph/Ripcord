...
/// What the last run of the listener wrote about how it ended.
public sealed record ListenerLogSummary(bool SawStart, ExitCode? Exit, bool Crashed, bool Cancelled)
{
...
/// The listener's own log, as `ripcord service` reads it back.
public static class ListenerLog
{
    ...
    public static IReadOnlyList<string> Candidates(string logsFolder, DateTimeOffset now) =>
    [
        WindowsPath.Join(logsFolder, LogFolder.FileName(DiagnosticOrigin.Listener, now)),
        WindowsPath.Join(
            logsFolder, LogFolder.FileName(DiagnosticOrigin.Listener, now.AddDays(-1))),
    ];
    ...
    /// the last run.
    public static ListenerLogSummary Summarise(IReadOnlyList<string> lines)
    {
        ...
        int banner = entries.FindLastIndex(
            entry => ListenerStartup.IsBanner(entry.Operation, entry.Message));
        ...
        return new ListenerLogSummary(banner >= 0, exit, crashed, cancelled);
    }
