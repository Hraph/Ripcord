...
It serves one thing, in one direction: the snapshot this host has published — `state.json`
beside `ripcord.yaml` by default, the file `ripcord status` writes. There is no verb,
