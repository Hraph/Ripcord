...
| `THIS HOST'S CERTIFICATE` | each certificate in `LocalMachine\My` with a private key, subject `CN=<this host>` — the subject the other host checks — and not expired, latest expiry first, marked `in ripcord.yaml` when it is the configured one. Shown even when `ripcord.yaml` does not load. Its thumbprint goes into `listener.local_certificate_thumbprint` on this host and `listener.peer_certificate_thumbprint` on the other one: run `ripcord service` on each host and cross the two values over. `NONE` when no certificate qualifies. |
