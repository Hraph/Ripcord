...
Not from Hyper-V. Each host publishes a snapshot of its own state to a file, and the listener
serves that file to the other host over mutual TLS ([`serve`](serve.md)). So `status` reads the
