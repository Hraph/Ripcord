...
sequence written by each. `failover`, `failback` and `reprotect` therefore **refuse** on a
version mismatch rather than warning — which means a pair left half-updated cannot be failed
over. The window between the two hosts is the one time the tool will not work.
