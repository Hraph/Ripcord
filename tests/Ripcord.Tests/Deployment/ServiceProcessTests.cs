...
public class ServiceProcessTests
{
    ...
    public void What_the_listener_writes_is_read_back() =>
        Assert.Equal(
            new ServiceProcess(Old, 4812),
            ServiceProcess.Parse(new ServiceProcess(Old, 4812).Text().Split('\n')));
    ...
    public void Anything_but_the_two_lines_is_not_a_record(string text) =>
        Assert.Null(ServiceProcess.Parse(text.Split('\n')));

    [Fact]
    public void A_service_that_is_not_running_has_no_build_to_report() =>
        Assert.Null(RunningBuild.Judge(
            Running with { State = ServiceRunState.Stopped }, new ServiceProcess(Old, 4812), New, Binary));

    [Fact]
    public void The_same_build_as_this_binary_is_current()
    {
        RunningBuild build = RunningBuild.Judge(Running, new ServiceProcess(New, 4812), New, Binary)!;
    ...
    {
        RunningBuild build = RunningBuild.Judge(Running, new ServiceProcess(Old, 4812), New, Binary)!;
    ...
    {
        RunningBuild build = RunningBuild.Judge(Running, new ServiceProcess(New, 1111), New, Binary)!;
    ...
    public void A_service_running_another_copy_is_not_outdated()
    {
        RunningBuild build = RunningBuild.Judge(
            Running, new ServiceProcess(Old, 4812), New, @"C:\Temp\ripcord.exe")!;
    ...
    {
        RunningBuild build = RunningBuild.Judge(
            Running with { ProcessId = null }, new ServiceProcess(New, 4812), New, Binary)!;
    ...
    public void A_current_or_unknown_build_says_nothing() =>
        Assert.All(
            new[]
            {
                RunningBuild.Judge(Running, new ServiceProcess(New, 4812), New, Binary),
